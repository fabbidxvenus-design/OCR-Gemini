...
test.describe('OCR latency Phase 1', () => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            imageUrl: null,
            ocrRaw: OCR_RESPONSE.ocrRaw,
            ocrStructured: OCR_RESPONSE.ocrStructured,
            tokenUsage: OCR_RESPONSE.tokenUsage,
            apiKeyIndex: OCR_RESPONSE.apiKeyIndex,
            edited: false,
            modelTier: 'default',
  ...
  test('persists thumbnail imageUrl in scan history after successful upload', async ({ page }) => {
    await page.addInitScript(() => {
      Object.defineProperty(navigator, 'mediaDevices', {
        value: {
          enumerateDevices: async () => [],
    ...
    await page.route('**/api/scans/upload-url', async (route) => {
      uploadUrlRequests += 1;
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          success: true,
          data: {
            uploadUrl: `https://storage.example.test/upload-${uploadUrlRequests}`,
            storagePath: `scans/user-1/thumb-${uploadUrlRequests}.jpg`,
    ...
    let createScanPayload: unknown;
    await page.route('**/api/scans', async (route) => {
      if (route.request().method() !== 'POST') {
        await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, data: [] }) });
        return;
      }
      createScanPayload = route.request().postDataJSON();
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          success: true,
          data: {
            id: 'remote-scan-thumb-1',
            timestamp: new Date().toISOString(),
            imageUrl: (createScanPayload as Record<string, unknown>)?.imageUrl ?? null,
            ocrRaw: OCR_RESPONSE.ocrRaw,
            ocrStructured: OCR_RESPONSE.ocrStructured,
            tokenUsage: OCR_RESPONSE.tokenUsage,
            apiKeyIndex: OCR_RESPONSE.apiKeyIndex,
    ...
    await expect.poll(() => uploadUrlRequests, { timeout: 10000 }).toBe(1);
    expect(createScanPayload).toHaveProperty('imageUrl', 'scans/user-1/thumb-1.jpg');
    expect(createScanPayload).not.toHaveProperty('imageDataUrl');
  });
});
